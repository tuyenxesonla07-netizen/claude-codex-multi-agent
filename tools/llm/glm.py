...
class GLMProvider(LLMProvider):
    """ZhipuAI GLM API Provider (OpenAI-compatible)."""

    def __init__(self, api_key: str = None, base_url: str = None,
                 model: str = "glm-4-flash", max_tokens: int = 8192):
        if not _HTTPX_AVAILABLE:
            raise ImportError("pip install httpx")
        self._api_key = api_key or os.environ.get("ZHIPU_API_KEY", "")
        if not self._api_key:
            raise ValueError("Set ZHIPU_API_KEY or pass api_key")
        self._base_url = (base_url or "https://open.bigmodel.cn/api/paas/v4").rstrip("/")
        self._model = model
