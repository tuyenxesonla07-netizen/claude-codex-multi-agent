...
class MiniMaxProvider(LLMProvider):
    """MiniMax API Provider (OpenAI-compatible)."""

    def __init__(self, api_key: str = None, base_url: str = None,
                 model: str = "abab6.5s-chat", max_tokens: int = 8192):
        if not _HTTPX_AVAILABLE:
            raise ImportError("pip install httpx")
        self._api_key = api_key or os.environ.get("MINIMAX_API_KEY", "")
        if not self._api_key:
            raise ValueError("Set MINIMAX_API_KEY or pass api_key")
        self._base_url = (base_url or "https://api.minimax.chat/v1").rstrip("/")
        self._model = model
